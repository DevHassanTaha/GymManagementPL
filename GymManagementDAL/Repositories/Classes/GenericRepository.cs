...
namespace GymManagementDAL.Repositories.Classes
{
    public class GenericRepository<TEntity> : IGenericRepository<TEntity> where TEntity : BaseEntity
    {
        ...
        public IEnumerable<TEntity> GetAll(Func<TEntity, bool>? conditioon = null)
        {
            if (conditioon is not null)
                return _context.Set<TEntity>().AsNoTracking().Where(conditioon).ToList();
            else
